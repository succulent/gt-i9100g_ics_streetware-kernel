...
static unsigned int sleep_max_freq=600000;
static unsigned int sleep_min_freq=200000;
static unsigned int sleep_prev_freq=200000;
static unsigned int sleep_prev_max=1200000;
...
static struct workqueue_struct	*kscary_wq;
...
{
	printk_once(KERN_INFO "CPUFREQ: scary sampling_rate_max "
...
/* cpufreq_scary Governor Tunables */
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	queue_delayed_work_on(cpu, kscary_wq, &dbs_info->work, delay);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	queue_delayed_work_on(dbs_info->cpu, kscary_wq, &dbs_info->work,
				delay);
}
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		if (dbs_enable == 1) {
			...
			/*
			 * scary does not implement micro like ondemand
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	kscary_wq = create_workqueue("kscary");
	if (!kscary_wq) {
		printk(KERN_ERR "Creation of kscary failed\n");
		return -EFAULT;
	}
    register_early_suspend(&scary_power_suspend);
	err = cpufreq_register_governor(&cpufreq_gov_scary);
	if (err)
		destroy_workqueue(kscary_wq);
